...
| 优化 | 名称 | 原理 | 性能提升 |
|:----:|------|------|:--------:|
| **1** | 并行除法器 | 3个除法器同时工作 | 速度 ×3 |
| **2** | CLA+流水线 | 进位预测 + 分级处理 | 频率 ×2 |
...
| `LongDivision_CLA_Pipeline_Group55.vhd` | 2 | 内置CLA快速减法器的流水线除法器 |
...
| `CLA_Pipeline_tb_Group55.vhd` | 2 | 对比 CLA vs RipSub 速度，验证流水线 |
...
处理N=7: [████████████]          Stage1  Stage2  Stage3  Stage4  Stage5
处理N=8:              [████]     
处理N=9:                  [██]   T1: [N=7]
                                 T2: [N=8]  [N=7]
总时间: 15周期 (3×5)             T3: [N=9]  [N=8]  [N=7]
                                 T4:        [N=9]  [N=8]  [N=7]
                                 T5:               [N=9]  [N=8]  [N=7]→完成!
...
### 5.3 Testbench 2: `CLA_Pipeline_tb_Group55.vhd` - CLA 与流水线验证

#### 测试目的

1. **Part 1**: 对比 CLA 减法器与 RipSub 减法器的速度
2. **Part 2**: 验证流水线架构的吞吐量提升
...
| 2 | 7,8,9 连续÷3 | Q=2,2,3 R=1,2,0 | 3个结果依次输出 |
...
| 时间 (ns) | Load | Dividend | Q_CLA | R_CLA | Done_CLA | 说明 |
|:---------:|:----:|:--------:|:-----:|:-----:|:--------:|------|
| 200 | 1 | 0111 (7) | X | X | 0 | 输入第1个数 |
| 220 | 1 | 1000 (8) | X | X | 0 | 输入第2个数 |
| 240 | 1 | 1001 (9) | X | X | 0 | 输入第3个数 |
| 260 | 0 | 1001 | X | X | 0 | 流水线处理中 |
| 280 | 0 | 1001 | X | X | 0 | 流水线处理中 |
| 300 | 0 | 1001 | **0010** | **0001** | **1** | ✓ 第1个结果: 7÷3=2余1 |
| 320 | 0 | 1001 | **0010** | **0010** | **1** | ✓ 第2个结果: 8÷3=2余2 |
| 340 | 0 | 1001 | **0011** | **0000** | **1** | ✓ 第3个结果: 9÷3=3余0 |

#### Part 2 流水线结果汇总

| 输入 | 商 Q_CLA | 余数 R_CLA | 验证 |
|:----:|:--------:|:----------:|:----:|
| 7÷3 | 0010 (2) | 0001 (1) | ✓ 正确 |
| 8÷3 | 0010 (2) | 0010 (2) | ✓ 正确 |
| 9÷3 | 0011 (3) | 0000 (0) | ✓ 正确 |

**验证成功标志**: 3个 `Done_CLA` 脉冲连续出现，间隔约 20ns (1个周期)
